...
public class ContentAPI {
    ...
    @Path("/search/{contentToSearch}")
    @GET
    @Produces("application/json")
    public List<DigitalContent> searchContents(@PathParam("contentToSearch") String contentToSearch, @QueryParam("partial") boolean isPartialSearchAllowed){
    	contentToSearch = contentToSearch.replaceAll("\\+","");
    ...
    @Path("/{contentKey}")
    @PUT
    public Response renameContent(@PathParam("contentKey") int contentKey, DigitalContent modifiedContent){
    	...
    	if (contentTable.renameContent(contentKey, modifiedContent)) {
